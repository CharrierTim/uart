...
| `O_START_BIT_ERROR` | std_logic                     |    out    | 0b0           | Output start bit error flag on `PAD_I_UART_RX` |
| `O_STOP_BIT_ERROR`  | std_logic                     |    out    | 0b0           | Output stop bit error flag on `PAD_I_UART_RX`  |
...
- `DDDDDDDD`: 32-bit data (eight ASCII hex characters)
...
- `AA`: two ASCII hex characters representing an 8-bit address (`0x00` .. `0xFF`)
...
Reply                  : 0F3C5555\r
Bytes received    (hex): 0x30 0x46 0x33 0x43 0x35 0x35 0x35 0x35 0x0D
```
...
Reply                  : DEADBEEF\r
Bytes received    (hex): 0x44 0x45 0x39 0x44 0x42 0x45 0x45 0x46 0x0D
```
...
- `AA`: two ASCII hex characters representing an 8-bit address (`0x00` .. `0xFF`)
- `DDDDDDDD`: eight ASCII hex characters representing the 32-bit data value to write, MSB first (`0x00000000` .. `0xFFFFFFFF`)
- `\r`: mandatory CR terminator (ASCII 0x0D)
- `\n`: optional LF (ASCII 0x0A), if present it is ignored by the device
...
```none
# Write value 0x12345678 to address 0x1A
Sent                   : W1A12345678\r\n
Bytes transmitted (hex): 0x57 0x31 0x41 0x31 0x32 0x33 0x34 0x35 0x36 0x37 0x38 0x0D 0x0A
Reply                  : (no reply expected)
```

```none
# Write value 0x00000001 to address 0x01
Sent                   : W0100000001\r
Bytes transmitted (hex): 0x57 0x30 0x31 0x30 0x30 0x30 0x30 0x30 0x30 0x31 0x0D
Reply                  : (no reply expected)
```
