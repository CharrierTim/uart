...
.. image:: _static/svg/UART-TB_TOP_FPGA.svg
...
.. list-table::
    ...
    - - **Type Name**
      - ``t_reg``
    - - **Purpose**
      - Register fields with address and default value
    - - **Fields**
      - - ``name`` : ``string`` - Register name identifier
...
.. list-table::
    ...
    - - ``C_GIT_ID``
      - vector[31:0]
      - 0x12345678
      - Git identifier for DUT version tracking
    - - ``C_BAUD_RATE_BPS``
      - positive
      - 0d115_200
    ...
    - - ``C_WRITE_NB_BITS``
      - positive
      - 10 * 8
      - Total bits for UART write command
    - - ``C_UART_WRITE_CMD_TIME``
      - time
      - ``C_BIT_TIME`` * ``C_WRITE_NB_BITS``
      - Total time for UART write command
    - - ``C_READ_NB_BITS``
      - positive
      - 10 * 9
      - Total bits for UART read command
    - - ``C_UART_READ_CMD_TIME``
      - time
      - ``C_BIT_TIME`` * ``C_READ_NB_BITS``
